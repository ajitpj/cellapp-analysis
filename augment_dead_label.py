...
mitotic/dead-like label from the HistGradientBoosting classifier
(models/dead_classifier_hgb.joblib).
...
crop and rewrites the cell_data sheet with updated dead_flag (1 = dead-like)
...
MODEL_PATH = Path(__file__).parent / "models" / "dead_classifier_hgb.joblib"


def augment_file(inference_dir: Path, model, suffix: str = "") -> None:
    ...
    instance_hits = sorted(inference_dir.glob("*_instance.tif"))
