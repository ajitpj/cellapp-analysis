...
def estimate_position_correction(stack,
    ...
    background_model : {"auto", "grid", "flatfield"}
        How the measured block values become a surface.

        ``grid``
            fill and smooth the measured grid - ~1000 free values. Follows
            real structure the flat field cannot express (uneven haze, a dirty
            coverslip), and is the better estimator whenever enough blocks
            survive.
        ``flatfield``
            fit `offset + level * F` per frame - two free values. Needs a
            `flatfield`, and only ten measurable blocks rather than a whole
            grid, so it still works where the grid model has nothing to
            interpolate between.
        ``auto`` (default)
            `grid`, unless fewer than `grid_min_blocks` of the blocks are
            measurable and a flat field is available, in which case
            `flatfield`. Also switches when any sampled frame has no
            measurable block at all, which the grid model cannot survive.

        The default is `auto` rather than `flatfield` because the constrained
        fit is a fallback, not an improvement: on the 20250213 plate the grid
        wins at every density that plate actually reaches (residual radial
        trend 9% against 25%). Its value is that it degrades instead of
        failing.
    grid_min_blocks : float
    ...
    if background_model not in ("auto", "flatfield", "grid"):
    ...
    if background_model == "auto":
        if flatfield is not None and usable < grid_min_blocks:
            chosen, why = "flatfield", (
    ...
    if chosen == "grid" and empty_frames:
        if flatfield is None:
            raise ValueError(
                f"{empty_frames} of {len(measured)} sampled frames have no "
                f"measurable background block, and there is no flat field to "
                f"fall back on. Lower `dilation`, or supply a flatfield so the "
                f'"flatfield" background model can be used.')
        chosen, why = "flatfield", (
            f"{empty_frames} of {len(measured)} sampled frames had no "
            f"measurable block at all")
    ...
    if chosen == "flatfield":
        ...
        # A frame with too few blocks to fit borrows the neighbouring fits;
        # the level moves smoothly, so interpolating it is safe where
        # inventing a whole surface is not.
        fits, fitted_at = [], []
        for n, g in enumerate(measured):
            try:
                fits.append(fit_background_to_flatfield(g, bshape))
                fitted_at.append(n)
            except ValueError:
                fits.append(None)
        if not fitted_at:
            raise ValueError(
                "no sampled frame has the 10 measurable blocks the flat-field "
                "background fit needs; this position is too crowded to correct")
        if len(fitted_at) < len(fits):
            fo = np.interp(np.arange(len(fits)), fitted_at,
                           [fits[n][0] for n in fitted_at])
            fl_ = np.interp(np.arange(len(fits)), fitted_at,
                            [fits[n][1] for n in fitted_at])
            fits = [f if f is not None else (fo[n], fl_[n])
    ...
    corr = PositionCorrection(
        ...
        diagnostics={
            ...
            "background_model": chosen,
            "background_model_requested": background_model,
            "background_model_reason": why,
            "usable_block_fraction": usable,
